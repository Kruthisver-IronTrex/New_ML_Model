...
# Free hosting — expanded with tumblr, canalblog, startspot (top hits from unclassified analysis)
_FREE    = re.compile(r"(000webhostapp|weebly|wixsite|blogspot|wordpress\.com|github\.io|netlify\.app"
                      r"|firebaseapp\.com|glitch\.me|repl\.co|web\.app|pages\.dev|vercel\.app|surge\.sh"
                      r"|byethost|infinityfree|tripod\.com|angelfire|jimdo|webnode|yolasite|ucoz|site123"
                      r"|strikingly|awardspace|tumblr\.com|canalblog\.com|over-blog\.com|startspot\."
                      r"|livejournal\.com|skyrock\.com|kazeo\.com|e-monsite\.com|eklablog\.com)", re.I)

_SHORT   = re.compile(r"^(bit\.ly|tinyurl\.com|goo\.gl|t\.co|ow\.ly|is\.gd|buff\.ly|shorte\.st|adf\.ly"
                      r"|rebrand\.ly|cutt\.ly|shorturl\.at|rb\.gy|tiny\.cc|lnkd\.in|clck\.ru|soo\.gd"
                      r"|bc\.vc|qrco\.de|bit\.do|t2m\.io|short\.io|bl\.ink|rotf\.lol|v\.gd|u\.to"
                      r"|tr\.im|tweez\.me|mcaf\.ee|x\.co|qr\.ae|url4\.eu|turl\.ca)$", re.I)

# Dynamic DNS — used for rapid IP rotation
_DDNS    = re.compile(r"(duckdns\.org|no-ip\.|ddns\.net|afraid\.org|bounceme\.net|hopto\.org|myftp\."
                      r"|myvnc\.|sytes\.net|zapto\.org|dynu\.com|dyn\.com|noip\.com|dyndns\.|changeip\."
                      r"|3utilities\.com|publicvm\.com|ddnsking\.com|freedns\.|gotdns\.|serveftp\."
                      r"|redirectme\.|serveblog\.|servehttp\.|onthewifi\.|serveminecraft\.)", re.I)

_BRANDS  = re.compile(r"(paypal|apple|google|microsoft|amazon|facebook|instagram|netflix|dropbox|linkedin"
                      r"|twitter|chase|wellsfargo|bankofamerica|ebay|yahoo|outlook|office365|onedrive"
                      r"|sharepoint|icloud|coinbase|binance|dhl|fedex|ups|usps|steam|roblox|discord"
                      r"|tiktok|whatsapp|telegram|citibank|hsbc|barclays|natwest|santander|lloyds"
                      r"|americanexpress|capitalone|robinhood|cashapp|venmo|stripe|shopify|spotify"
                      r"|adobe|zoom|intuit|turbotax)", re.I)

_TYPOS   = re.compile(r"(paypa[l1][^a-z]|g[o0][o0]gle|micr[o0]s[o0]ft|faceb[o0]{2}k|amaz[o0]n"
                      r"|app[l1]e(?!t)|netfl[i1]x|[l1]inked[i1]n|tw[i1]tter|dr[o0]pb[o0]x|paypall"
                      r"|g[o0]{2}g[l1]e|y[a4]h[o0]{2}|[e3]bay|[i1]nstagramm?|wh[a4]ts[a4]pp"
                      r"|d[i1]sc[o0]rd|g00g1e|micros0ft|facebo0k|t[i1]kt[o0]k)", re.I)

_LOOK    = re.compile(r"(pay[-_]?pa[l1]|g[o0]{2,}gle|micros[o0]ft|arna[z2]on|amaz[o0][n0]|faceb[o0]{2,}k"
                      r"|app[l1][e3]|[l1]inked[l1]n|tw[i1]tt[e3]r|y[a4]ho{2}|[e3]b[a4]y|d[i1]scord"
                      r"|st[e3][a4]m|r[o0]b[l1][o0]x|[i1]cl[o0]ud|sh[o0]p[i1]fy|sp[o0]t[i1]fy"
                      r"|wh[a4]tsapp|t[i1]kt[o0]k|[o0]utl[o0]{2}k)", re.I)

_MALEXT  = re.compile(r"\.(exe|zip|rar|7z|msi|bat|ps1|sh|apk|dmg|iso|jar|vbs|cmd|scr|pif|hta|wsf"
                      r"|lnk|dll|cab|deb|rpm|xll|xlam|docm|xlsm|pptm)(\?.*)?$", re.I)

_HOMO    = re.compile(r"[\u00C0-\u024F\u0370-\u03FF\u0400-\u04FF]")

_CRED    = re.compile(r"(login|signin|sign[-_]in|log[-_]in|verify|secure|account|update|confirm"
                      r"|password|credential|auth(?:enticate)?|banking|wallet|portal|webscr|validation"
                      r"|security|recover|unlock|suspend|billing|invoice|checkout|username|passwd|reset"
                      r"|reactivate|2fa|otp|token|identity|ssn|cvv|expir)", re.I)

_REDIR   = re.compile(r"(?:^|&)(?:redirect|redir|url|next|goto|return(?:url|to)?|rurl|dest(?:ination)?"
                      r"|continue|forward|target|callback|fallback|successurl|cancelurl)=", re.I)

_SUSP_W  = re.compile(r"(free|prize|winner|lucky|reward|gift|bonus|deal|offer|promo|coupon|discount"
                      r"|limited|urgent|alert|warning|suspended|verify|unusual|activity|required"
                      r"|expire[ds]?|blocked|frozen|restricted|compromised|unauthorized"
                      r"|action.required|confirm.now)", re.I)

# Suspicious TLDs — abused heavily in phishing
_STLDS   = re.compile(r"\.(xyz|top|club|online|site|tk|ml|ga|cf|gq|pw|cc|ws|buzz|icu|cyou|fun|monster"
                      r"|rest|uno|works|click|link|win|loan|gdn|accountant|review|trade|science|date"
                      r"|faith|racing|bid|party|cricket|download|stream|vip|live|shop|store|tech"
                      r"|digital|space|zone|rocks|guru|ninja|ru|cn|su|to|la)$", re.I)
...
_AT      = re.compile(r"https?://[^@\s]{1,256}@")
_IPHOST  = re.compile(r"^(\d{1,3}\.){3}\d{1,3}$")
_CMS     = re.compile(r"/(wp-(?:login|admin|content|includes|config|json|cron)|phpmyadmin|xmlrpc\.php"
                      r"|administrator|cpanel|joomla|drupal|magento|prestashop|opencart|typo3)(?:/|\.php|$)", re.I)

# Webmail — clone of real webmail interfaces to steal email creds
_WMAIL   = re.compile(r"(webmail\.|roundcube|squirrelmail|horde|zimbra|owa\.|autodiscover\.|rainloop"
                      r"|afterlogic|mail\.|smtp\.|imap\.|pop3\.)", re.I)

# Adult content — used as bait or age-gate credential harvesting
_ADULT   = re.compile(r"(porn|(?<![a-z])sex(?![a-z])|nude|naked|xxx|erotic|hentai|milf|cumshot|fetish"
                      r"|bdsm|stripclub|escort|hookup|onlyfan|camgirl|nsfw|xvideo|xhamster|redtube"
                      r"|youporn|pornhub|brazzers|bangbros|sextracker|startspot\.nl)", re.I)
...
_PATHURL = re.compile(r"https?://", re.I)
_TRKSUB  = re.compile(r"^(track|click|redirect|trk|go|out|link|ad|ads|promo|pixel|imp|beacon|stat|hit)\.", re.I)
_LEGITTR = re.compile(r"(google-analytics\.com|googletagmanager\.com|segment\.com|amplitude\.com"
                      r"|mixpanel\.com|hotjar\.com|cloudflare\.com|fastly\.com|akamai|cloudfront\.net)", re.I)

# Localhost / loopback — phishing kit artefacts
_LOCAL   = re.compile(r"^(localhost|127\.0\.0\.1|0\.0\.0\.0|::1)$", re.I)
...
def _parse(raw):
    ...
    try:
        ...
        ent           = _entropy(dom)
        return {
            "url": url, "host": host, "sub": sub, "dom": dom, "suf": suf,
            "path": path, "query": query, "pq": path + "?" + query, "port": p.port,
            "ent": ent, "dlen": len(dom), "ulen": len(url),
            "drat": sum(c.isdigit() for c in dom) / len(dom) if dom else 0,
            "enc": len(_ENC.findall(url)), "depth": path.count("/"),
            "skw": len(_SUSP_W.findall(url)),
        }
    except Exception:
        return None
...
def r_free(f):     return bool(_FREE.search(f["host"]))
def r_short(f):    return bool(_SHORT.match(f["host"]))
def r_ddns(f):     return bool(_DDNS.search(f["host"]))                          # NEW: was missing
def r_ip(f):       return bool(_IPHOST.match(f["host"])) and f["port"] is None
def r_ipp(f):      return bool(_IPHOST.match(f["host"])) and f["port"] is not None
def r_port(f):     return f["port"] not in (None, 80, 443) and not bool(_IPHOST.match(f["host"]))
def r_local(f):    return bool(_LOCAL.match(f["host"]))                           # NEW: localhost URLs
def r_ipfs(f):     return bool(_IPFS.search(f["url"]))
def r_cms(f):      return bool(_CMS.search(f["url"]))
def r_hexsub(f):   return bool(_HEXSUB.match(f["sub"]))
def r_numsub(f):   return bool(_NUMLONG.match(f["sub"]))
...
def r_pathc(f):    return bool(_PATHURL.search(unquote(f["path"]))) or bool(re.search(r"https?%3A", f["path"], re.I))
def r_redir(f):    return bool(_REDIR.search(f["query"] + "&" + f["url"])) and _external_redirect(f["url"], f["host"])
def r_track(f):    # Tracking subdomain + at least one suspicious signal — avoids analytics FPs
    return (not bool(_LEGITTR.search(f["host"])) and bool(_TRKSUB.match(f["sub"] + "."))
            and (bool(_STLDS.search("." + f["suf"])) or f["skw"] >= 1 or bool(_FREE.search(f["host"]))))
def r_data(f):     return bool(_DATAURI.match(f["url"]))
def r_malext(f):   return bool(_MALEXT.search(f["path"]))
...
def r_cred(f):     return bool(_CRED.search(f["pq"])) or f["skw"] >= 2
def r_dpbrand(f):  return bool(_BRANDS.search(f["path"])) and f["depth"] >= 3
def r_wmail(f):    return bool(_WMAIL.search(f["host"])) and not bool(_BRANDS.search(f["dom"]))  # NEW: webmail clones
def r_adult(f):    return bool(_ADULT.search(f["url"]))                           # NEW: adult bait content
def r_stacksub(f): return f["host"].count(".") >= 4 or f["sub"].count(".") >= 2

# ── Heuristic ─────────────────────────────────────────
# Requires ALL THREE — entropy + suspicious TLD + length (prevents .com false positives)
def r_rand(f):     return f["ent"] > 3.5 and bool(_STLDS.search("." + f["suf"])) and f["dlen"] > 8
...
# ══════════════════════════════════════════════════════
# 6. SECONDARY CLASSIFIER (≤2 tags, weaker thresholds)
#    Only runs when ZERO primary rules matched
# ══════════════════════════════════════════════════════

def _secondary(f):
    out = []
    # Relaxed entropy — catches DGA domains on common TLDs like .com
    if f["ent"] > 3.8 and f["dlen"] > 8:
        out.append("Random-Suspicious-Domain")
    # Single cred keyword (primary requires full path+query match or ≥2 words)
    if len(out) < 2 and (bool(_CRED.search(f["pq"])) or f["skw"] == 1):
        out.append("Credential-Harvesting")
    # Suspicious TLD alone — weaker signal than full random-domain triple criteria
    if len(out) < 2 and bool(_STLDS.search("." + f["suf"])) and "Random-Suspicious-Domain" not in out:
        out.append("Random-Suspicious-Domain")
    # Brand in URL but not in registered domain — weak subdomain confusion
    if len(out) < 2 and bool(_BRANDS.search(f["url"])) and not bool(_BRANDS.search(f["dom"])):
        out.append("Brand-Subdomain")
    # High digit ratio in domain — generated/numeric domain pattern
    if len(out) < 2 and f["drat"] > 0.5 and f["dlen"] > 6 and "Random-Suspicious-Domain" not in out:
        out.append("Random-Suspicious-Domain")
    # Adult keywords missed by primary (e.g. shorter matches, subdomain only)
    if len(out) < 2 and bool(_ADULT.search(f["url"])):
        out.append("Adult-Content")
    # Dynamic DNS missed by primary (host parsed differently)
    if len(out) < 2 and bool(_DDNS.search(f["host"])):
        out.append("Dynamic-DNS")
    return out[:2]
...
for raw in tqdm(phishing, desc="Classifying", unit=
"url"):
    f = _parse(raw)
    if f is None:
        counts["Unclassified"] += 1; unclassified.append(raw); continue
...
if unclassified:
    def _info(raw):
        ...
        if f: return {"url": f["url"], "domain": f["host"], "tld": f["suf"], "length": f["ulen"],
                      ...
                      "path_depth": f["depth"], "enc_count": f["enc"], "susp_keywords": f["skw"],
                      "has_ip": bool(_IPHOST.match(f["host"])), "subdomain_dots": f["host"].count(".")}
        return {"url": raw, "domain": None, "tld": None, "length": len(raw), "entropy": 0,
                "digit_ratio": 0, "path_depth": 0, "enc_count": 0, "susp_keywords": 0,
                "has_ip": False, "subdomain_dots": 0}
    pd.DataFrame([_info(u) for u in tqdm(unclassified, desc="Saving unclassified", unit="url")]
